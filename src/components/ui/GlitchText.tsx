...
const GlitchText: React.FC<GlitchTextProps> = ({ text, as: Tag = 'span', className = '' }) => {
  return (
    <Tag className={`relative inline-block group font-mono ${className}`}>
      ...
      <span className="absolute top-0 left-0 -z-10 w-full h-full text-cyber-green opacity-0 group-hover:opacity-70 group-hover:translate-x-[2px] transition-all duration-100 select-none">
        {text}
      </span>
